...
for imno = 1:noimages
    ...
    for ringno = 1:norings
        ringmask = (ringno-1) == RQ;%this is a logical array defining the ring
        A = abs(ringmask * cftplan);
        absfftsums(imno,ringno) = sum(A(:))/ptsperring(ringno);% average over Fourier magnitude in ring
    end
end
...
for imno = 1:noimages
    immean = FStack(:, :, imno);
    meanfreqcontent(imno) = sum(absfftsums(imno, :) .* w) / mean(immean(:)) ; % combine w and absfftsums
end
