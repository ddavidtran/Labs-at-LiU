	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				double new_Qstate_value = Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction)) * (previous_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state) - Qtable.get(prev_stateaction));
				//System.out.println(new_Qstate_value);
				Qtable.put(prev_stateaction, new_Qstate_value);
				/* See top for constants and below for helper functions */
				//System.out.println(prev_stateaction);
				...
				if (print_counter % 10 == 0) {
					System.out.println("ITERATION: " + iteration + " SENSORS: a=" + df.format(57.2957795*angle.getValue()) + " vx=" + df.format(vx.getValue()) + 
