...
nbrTestExamples = 50;
maxWeight = 5; % Times the initial weight
...
plot(faultCounter, 'o')
ylim([-1, inf])
[M, I] = maxk(faultCounter,4);

figure(3)
for k=1:4
    if I(k) <= nbrTestExamples
        %figure(2 + k)
        subplot(2,2,k), imagesc(faces(:,:,10*I(k)));
        axis image;
        axis off;
    end
end
...
for k=1:4
    if I(k) > nbrTestExamples
        %figure(7 + k)
        subplot(2,2,k), imagesc(nonfaces(:,:,10*I(k)));
        axis image;
        axis off;
    end
end
